...
export default function SingleAssignmentButtons({ currentAssignment }: any) {
  ...
  const handleDelete = async () => {
    try {
      ...
      // Dispatch the delete action only if the deletion is successful
      dispatch(deleteAssignment(currentAssignment));
    } catch (error) {
      // Log the error for debugging
      console.error("Error deleting assignment:", error);
    } finally {
      // Always close the modal and navigate back to the assignments page
      setShowModal(false);
    ...
    <div className="float-end">
      ...
      <Modal show={showModal} onHide={() => setShowModal(false)} centered>
        ...
        <Modal.Body>
          ...
          <span className="text-danger fw-bold">remove</span> the assignment:{" "}
